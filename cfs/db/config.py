...
from cfsCore.interface import CollectionDB


class CFSconfig:
    """
    Handles interaction with configuration. There are two types of configuration of
    interest: user based configuration, and web service based configuration.
    """
    def __init__(self, filename=None):
        ...
        self.prootocol = self.config['_DB']['db_protocol']
        self._db = CollectionDB(self.protocol)
        self._db.init(self.conn_string)

    @property
    def conn_string(self):
        """ Return what the current configuration thinks is the connection string"""
        return self.protocol + self.config['_DB']['db']
...
[_DB]
db = cfstore.django.db
db_protocol = standalone
last_collection =  
last_location = 
